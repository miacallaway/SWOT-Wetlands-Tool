'''
Functions to extract SWOT Water Surface Elevation (WSE) from the SWOT Pixel Cloud product and plot water height time series relative to a gauge (Australian Bureau of Meteorology (BOM) gauge text files), 
spatially plot gridded water height or water height anomalies. Functions are also provided to compute spatial water depth where LiDAR data or a DEM is available. 
...
def full_site_water_depth(directory, file_list, elevation_grid, min_x, min_y, max_x, max_y, wetland_cross_sections, size=100, wetland_mask=mask_with_no_prior_included):
    ...
    the gridded SWOT water height data to obtain water depth, with negative depth values removed. Wetland cross-sections are shown over the spatial plot for the first file. 
    The spatial plots are saved to a .png file which is labelled based on the SWOT file date.
    ...
    Inputs:
        ...
        wetland_cross_sections: dictionary of linestrings (cross-sections used in plots of cross-sectional water height relative to LiDAR derived bathymetry)
    ...
    for i, filename in enumerate(file_list):
        ...
        lat_x_edges, _ = transformer_reverse.transform(x_edges, np.full_like(x_edges, np.min(y_edges)))
        _, lon_y_edges = transformer_reverse.transform(np.full_like(y_edges, np.min(x_edges)), y_edges)

        ax.imshow(img, extent=img_extent)
        mesh = ax.pcolormesh(lat_x_edges, lon_y_edges, np.flipud(depth), shading='auto', cmap='Blues', vmin=0, vmax=6)
        ...
        plt.tight_layout()
        plt.savefig(f'{save_folder}/CL_full_gridded_depth_{str(date_obj)[0:10]}.png', bbox_inches='tight', pad_inches=0.1, dpi=600)
        plt.show()
    return None
...
def point_cloud_to_df(las_directory, polygon, output_file):
    ...
    transformed_polygon = Polygon([transformer.transform(lat, lon) for lat, lon in polygon.exterior.coords])
...
def extract_elevation_transect(wetland_cross_sections, wetland_name_list, lidar_file, buffer_radius=200):
    ...
    for wetland_name in wetland_name_list:
        ...
        mask = vectorized.contains(buffer, df['x'].values, df['y'].values)
        lidar_df = df[mask]
        ...
        print(wetland_name, cum_distances, elevation)
...
def cross_sectional_wse_topography(linestring_dictionary, file_list, wetland_mask, buffer_radius=200):
    '''
    Calculates and plots the average area-weighted water height of a series of wetland cross-sections relative to the bathymetry for a given list 
    ...
    Inputs: 
        linestring_dictionary: dictionary of linestrings for a series of wetlands to calculate cross-sectional water height along
        file_list: list of SWOT files to calculate and plot cross-sectional water height for
        wetland_mask: mask to filter out poor quality data (default=mask_with_no_prior_included)
        buffer_radius: sampling radius of points to calculate an average value from (default=200 m)
    '''
    ...
    for i, filename in enumerate(file_list):
        ...
        for i, wetland_name in enumerate(linestring_dictionary):
            ...
            with open(f'{save_folder}/site_data_{str(date_obj)[0:10]}_{wetland_name}_without_noprior.pkl', "wb") as f:
                pickle.dump(site_data, f)

            with open(f'{save_folder}/site_data_{str(date_obj)[0:10]}_{wetland_name}_without_noprior.pkl', "rb") as f:
                site_data = pickle.load(f)

            x_offset = 0  
            for i, (x, y, dur) in enumerate(site_data):
                x_shifted = np.asarray(x) + x_offset
                plt.plot(x_shifted, y, linewidth=1.5, color = time_colour, label=label, zorder=1)
                x_offset += dur + gap
...
def cross_sectional_wse_topography_from_saved(linestring_dictionary, file_list, buffer_radius=200):
    '''
    Plots the average area-weighted water height of a series of wetland cross-sections relative to the bathymetry for a given list 
    ...
    plt.xlabel('Distance along each wetland transect (km)', fontsize=20)
...
#######################################
## Wetland Water Height Time Series  ##
#######################################

def extract_wetland_pixc_wse(directory, wetland_polygons, VerC_mask, VerD_mask):
    '''
    Reads in all parameters, extracts values, masks and filters the values to remove poor quality data and create dataframes of water height and standard deviation 
    ...
    files are merged before undertaking masking, filtering and calculation of water heights and standard deviations. 
    ...
    Returns: 
        wetland_dataframes: list of dataframes (in order of wetlands in wetland_polygon dictionary) containing the date of SWOT data, area-weighted average water height, standard deviation, area-weighted water height with KDE filtering and standard deviation of KDE filtered pixel water heights  
...
def wetland_time_series(directory, wetland_polygons, VerC_mask, VerD_mask, fstring_save_to_file):
    '''
    Creates a series of dataframes of water height and standard deviation over time for SWOT data files in a given directory and save each dataframe to a .parquet file containing the wetland polygon name in the file name. 
...
def plot_time_series(wetland_list, gauge_list, new_mask_filename, maubant_mask_filename, wrmse=True, label='In situ gauge', threshold_days=1):
    '''
    Plots the water height time series from the new mask and the Maubant et al. (2025) mask against a gauge time series and saves the outputs for each wetland and gauge. 
    ...
    the adjusted SWOT water height values relative to the gauge is computed. Each of these plots are saved to a .png file with the wetland polygon name noted in the file name. 
    ...
    Inputs:
        ...
        threshold_days: maximum number of days between the SWOT and gauge data to compare a SWOT data point with the closest gauge water height for computing the absolute bias (default=1 day)
    '''
    ...
    for wetland_poly, gauge in zip(wetland_list, gauge_list):
        ...
        for i, (mask_name, df_b) in enumerate(swot_time_series.items()):
            ...
            if i==0:
                plt.errorbar(df_b['Date'], df_b['WSE_KDE'] - bias, yerr=df_b['ST_DEV_KDE'], fmt='o', capsize=2, label=f'{mask_name}: WRMSE ({weighted_rmse:.2f} m)' if wrmse is True else f'{wetland_poly}: {mask_name}', color=colours[i])
            elif i==1:
                plt.errorbar(df_b['Date'], df_b['WSE'] - bias, yerr=df_b['ST_DEV'], fmt='o', capsize=2, label=f'{mask_name}: WRMSE ({weighted_rmse:.2f} m)'if wrmse is True else f'{wetland_poly}: {mask_name}', color=colours[i])
...
def plot_all_time_series_gauge_discharge(gauge_filename, discharge_filename, wetland_polygons, timeseries_filename, threshold_days=1, connecting_threshold=21):
    '''
    Plots the wetland polygons over the study site, plot the water height time series of all wetlands (adjusted for absolute bias relative to the plotted gauge) with a nearby 
    ...
    for i, wetland_poly in enumerate(wetland_polygons):
        masked = pd.read_parquet(f'{timeseries_filename}_{wetland_poly}.parquet')
...
def gridded_wetland_height_anomaly(directory, file_list, wetland_polygons, mask_function):
    '''
    Calculates and plots gridded water height and gridded water height anomalies (relative to the mean gridded water height) for a given list of files and wetland polygons. 
    Histograms of pixel level and grid level water height are also plotted. All plots are saved to .png files using the file date, wetland name and plot type. 
            ...
            if len(latitude) > 0:
                ...
                if len(filtered_df) > 5:
                    for size in [100]:                        
                        ...
                        sc = plt.scatter(filtered_df['Longitude'], filtered_df['Latitude'], c=pixel_height_anomaly, cmap=blue_white_green_cmap, s=0.5, vmin=-0.2, vmax=0.2)
                        cbar=plt.colorbar(sc)
                        cbar.set_label(label='Pixel water height anomaly (m)', fontsize=11)
                        ...
                        fig, ax = plt.subplots(1,2, figsize=(9,2), constrained_layout=True)
                        ...
                        if not np.isnan(weighted_avg).all():
                            ...
                            # print(str(date_obj)[0:10], wetland_poly, np.nanmean(outlier_removed), len(outlier_removed[~np.isnan(outlier_removed)])) # Uncomment if needing area/grid count and average water height information for computing volume
                            ...
                            fig.supxlabel('Water height anomaly (m)', fontsize=11)
                            ...
                            for variable, label, cmap in zip([outlier_removed, gridded_height_anomaly], ['Water height (m)', 'Water height anomaly (m)'], ['Blues', blue_white_green_cmap]):
                                fig, ax = plt.subplots()
                                ax.imshow(img, extent=img_extent) # Extent matched to actual coordinates
                                ...
                                lat_x_edges, _ = transformer_reverse.transform(x_edges, np.full_like(x_edges, np.min(y_edges)))
                                _, lon_y_edges = transformer_reverse.transform(np.full_like(y_edges, np.min(x_edges)), y_edges)
                                mesh = ax.pcolormesh(lat_x_edges, lon_y_edges, np.flipud(variable), shading='auto', cmap=cmap)
                                cbar = plt.colorbar(mesh, ax=ax)
...
##########################
## Gridded Water Height ##
##########################

def compute_gridded_wetland_standard_deviation(directory, wetland_polygons, mask_function, save_to_filename):
    '''
    Computes the standard deviation of masked, filtered and gridded SWOT water heights for grid cell sizes between 50 and 500 m inclusive at 50 m intervals for each wetland polygon over time. 
    ...
    Returns: 
        wetland_dataframes: list of wetland dataframes containing the standard deviations of the gridded wetland water height for different grid sizes at each epoch with full spatial coverage
    '''
    wetland_bounds = get_wetland_bounding_box(wetland_polygons)
            ...
            if len(latitude) > 0:
                ...
                if len(filtered_df) >= 5:
                    for size in [50, 100, 150, 200, 250, 300, 350, 400, 450, 500]:
                        ...
                        weighted = np.full_like(sum_height, np.nan, dtype=float)
                        weighted_avg = np.divide(sum_height, sum_area, out=weighted, where=sum_area!=0) # Area-weighted average water height
