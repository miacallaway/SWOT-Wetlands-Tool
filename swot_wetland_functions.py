            ...
            if len(latitude) > 0:
                ...
                if len(filtered_df) >= 5:
                    for size in [100]:                        
                        ...
                        print(len(polygon_df['height']), len(filtered_df['height']))
                        fig, ax = plt.subplots(figsize=(10,2), constrained_layout=True)
                        counts, bins, patches = ax.hist(polygon_df['height'], bins=np.arange(np.min(polygon_df['height']), np.max(polygon_df['height']), 0.1), edgecolor='black', color='red', label='Removed by KDE filtering')
                        counts, bins, patches = ax.hist(filtered_df['height'], bins=np.arange(np.min(polygon_df['height']), np.max(polygon_df['height']), 0.1), edgecolor='black', color='blue', label='Retained')
                        plt.ylabel('Pixel frequency', fontsize=11)
                        plt.legend()
                        plt.show()
