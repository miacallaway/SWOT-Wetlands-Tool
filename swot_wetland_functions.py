...
The functions were created during the production of the paper 'Surface Water and Ocean Topography satellite data reveals flood depths and environmental flows in wetlands' - Callaway et al., accepted in principle June 2026
...
Authors: Mia Callaway, Maya Taib, Paul Tregoning and Lachlan Dodd
...
from shapely import Polygon, STRtree, contains_xy, box, Polygon, Point, LineString
...
wgs84_to_gda94_ahd = Transformer.from_pipeline(wgs84_to_gda94_ahd_pipeline)          # Takes in lat/lon/height
...
def lidar_average_grid(las_dir, grid_bounds, cell_size, output_npy_path):
    '''
    Creates a grid of average ground elevation from a LiDAR point cloud
    ...
    Inputs:
        ...
        cell_size: gridding cell size resolution (in m)
        output_npy_path: path to save .npy file containing averaged LiDAR grid to

    Returns: averaged ground elevation grid
    '''
    min_x, min_y, max_x, max_y = grid_bounds
    width = int(np.ceil((max_x - min_x) / cell_size))
    height = int(np.ceil((max_y - min_y) / cell_size))
    ...
    for file in tqdm(os.listdir(las_dir), desc="Processing LAS files"):
        ...
        try:
            with laspy.open(filepath) as fh:
                ...
                x = x[classification == 2]
                y = y[classification == 2]
                z = z[classification == 2]
                ...
                flat_idx = row_indices * width + col_indices
                np.add.at(sum_grid.ravel(), flat_idx, z)
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        average_grid = sum_grid / count_grid
        average_grid[count_grid == 0] = np.nan

    np.save(output_npy_path, average_grid)
...
def process_transformed_swot_data(file, wetland_mask):
    ...
    if len(lon) > 0:
        ...
        gda_lon, gda_lat, swot_ahd = wgs84_to_gda94_ahd.transform(lat, lon, h)
...
def full_site_water_depth(directory, file_list, elevation_grid, min_x, min_y, max_x, max_y, wetland_cross_sections, size=100, wetland_mask=mask_with_no_prior_included, plot_wse=False, plot_elevation=False):
    '''
    ...
    Inputs:
        ...
        plot_wse: plot gridded WSE across the same area (default=False)
        plot_elevation: plot gridded elevation across the same area (default=False)
    '''
    for i, filename in enumerate(file_list):
        ...
        if plot_wse is True:
            ...
            cbar.set_label('Water Height (m)', fontsize=12)
            plt.xlim(xmin, xmax)
            plt.ylim(ymin, ymax)
            plt.xlabel('Longitude', fontsize=12)
            plt.ylabel('Latitude', fontsize=12)
            plt.tight_layout()
            plt.savefig(f'{save_folder}/full_gridded_wse_{str(date_obj)[0:10]}_{size}m.png', bbox_inches='tight', pad_inches=0.1, dpi=600)
            plt.show()

        if plot_elevation is True:
            fig, ax = plt.subplots()
            ax.imshow(img, extent=img_extent)
            ...
            plt.ylabel('Latitude', fontsize=12)
            plt.tight_layout()
            plt.savefig(f'{save_folder}/full_gridded_elevation_{str(date_obj)[0:10]}_{size}m.png', bbox_inches='tight', pad_inches=0.1, dpi=600)
            plt.show()
...
def extract_elevation_transect(wetland_cross_sections, wetland_name_list, lidar_file, buffer_radius=200):
    ...
    for wetland_name in wetland_name_list:
        ...
        mask = contains_xy(buffer, df['x'].astype(float), df['y'].astype(float))
        lidar_df = df[mask]
...
def cross_sectional_wse_topography(linestring_dictionary, file_list, wetland_mask, buffer_radius=200):
    '''
    Calculates the average area-weighted water surface elevation of a series of wetland cross-sections relative to the bathymetry for a given list 
    of SWOT files. SWOT data from each date is saved to a .pkl file labelled by date and wetland name. 
        ...
        for i, wetland_name in enumerate(linestring_dictionary):
            ...
            for pt in sample_points:
                ...
                if np.any(inside_mask):
                    ...
                    pixel_wse = h[inside_mask].compressed()
                else:
                    mean_val = np.nan
                wse.append(mean_val)
                pixel_wse_list.append(pixel_wse)

            cum_distances = [line.project(pt) for pt in sample_points]
            cross_section_distance = np.max(cum_distances)
            site_data.append((cum_distances, wse, cross_section_distance, pixel_wse_list))
            
            with open(f'{save_folder}/site_data_{str(date_obj)[0:10]}.pkl', "wb") as f:
                pickle.dump(site_data, f)
    return None

def cross_sectional_wse_topography_from_saved(linestring_dictionary, file_list, buffer_radius=200, plot_spread=False, ymin=None, ymax=None):
    '''
    ...
    Inputs: 
        ...
        file_list: list of SWOT files to calculate and plot cross-sectional water surface elevation for
        buffer_radius: sampling radius of points to calculate an average value from (default=200 m)
        plot_spread: plot individual values which are averaged to obtain the transect lines (default=False) 
        ymin: lower y bound for plot (default=None)
        ymax: upper y bound for plot (default=None)
        '''
    ...
    for i, wetland_name in enumerate(linestring_dictionary):
        ...
        bathymetry_data.append((elevation_data[0,0], elevation_data[0,1], cross_section_distance))

    for i, (x, y, dur) in enumerate(bathymetry_data):
        x_shifted = np.asarray(x) + x_offset
        plt.plot(x_shifted, y, linewidth=1.5, color='black', label='Bathymetry' if i==0 else None, zorder=4)
        ...
        plt.axvline(x=x_offset - gap/2, color='gray', linestyle=':', linewidth=2, zorder=5) # Draw dotted vertical separators
    ...
    for i, filename in enumerate(file_list):
        ...
        with open(f'{save_folder}/site_data_{str(date_obj)[0:10]}.pkl', "rb") as f:
            site_data = pickle.load(f)

        save_data = []        
        x_offset = 0  
        for i, ((x, y, dur, pixel_wse_list), bathymetry) in enumerate(zip(site_data, bathymetry_data)):
            y = [mean_wse if mean_wse > bathymetry_value else np.nan for mean_wse, bathymetry_value in zip(y, bathymetry[1])]
            pixel_wse_list = [pixel_wse if mean_wse > bathymetry_value else [np.nan] for mean_wse, pixel_wse, bathymetry_value in zip(y, pixel_wse_list, bathymetry[1])]
            save_data.append((x, y, dur, pixel_wse_list))

            x_shifted = np.asarray(x) + x_offset
            plt.plot(x_shifted, y, linewidth=3, color=time_colour, label=str(date_obj)[0:10] if i==0 else None, zorder=2)
            if plot_spread is True:
                for i in range(len(x_shifted)):
                    plt.scatter([x_shifted[i]] * len(pixel_wse_list[i]), pixel_wse_list[i], s=0.5, alpha=0.05, color=time_colour, zorder=1)
            x_offset += dur + gap

        with open(f'{save_folder}/transect_wse_data_{str(date_obj)[0:10]}.pkl', "wb") as f:
    ...
    if plot_spread is False:
        plt.savefig(f'{save_folder}/fig1_cross_section_{str(date_obj)[0:4]}.png', bbox_inches='tight', pad_inches=0.2, dpi=900)
    else:
        plt.savefig(f'{save_folder}/fig1_cross_section_{str(date_obj)[0:4]}_plot_spread.png', bbox_inches='tight', pad_inches=0.2, dpi=900)
    plt.show()
    return None
...
def extract_wetland_pixc_wse(directory, wetland_polygons, VerC_mask, VerD_mask):
    ...
    '''
    wetland_dataframes = [pd.DataFrame(columns=["Date", "WSE", "ST_DEV", "PIXEL_WSE", "WSE_KDE", "ST_DEV_KDE", "PIXEL_WSE_KDE"]) for _ in range(len(wetland_polygons))]
    ...
    while i < len(files):
        ...
        for wetland_poly, wetland_df, bounds in zip(wetland_polygons, wetland_dataframes, wetland_bounds):
            ...
            if math.isnan(Good_Mean) == False:
                wetland_df.loc[len(wetland_df)] = [date_obj, Good_Mean, np.nanstd(Good_Pixel_Heights), Good_Pixel_Heights, KDE_Mean, np.nanstd(KDE_Pixel_Heights), KDE_Pixel_Heights]
...
def plot_time_series(wetland_list, gauge_list, new_mask_filename, maubant_mask_filename, wrmse=True, maubant=True, label='In situ gauge', threshold_days=1, plot_spread=False, ymin=None, ymax=None):
    '''
    ...
    for wetland_poly, gauge in zip(wetland_list, gauge_list):
            ...
            if i==0:
                ...
                if plot_spread is True:
                    for idx, row in df_b.iterrows():
                        plt.scatter([x + timedelta(days=np.random.uniform(-1, 1)) for x in [row['Date']] * len(row['PIXEL_WSE_KDE'])], row['PIXEL_WSE_KDE'] - bias, s=2, alpha=0.05, color="lightcoral")                
                save_df = pd.DataFrame({'Date': df_b['Date'], 'WSE_KDE': df_b['WSE_KDE'] - bias, 'ST_DEV_KDE': df_b['ST_DEV_KDE'], 'PIXEL_WSE_KDE': [row['PIXEL_WSE_KDE'] - bias for idx, row in df_b.iterrows()]})
                save_df.to_parquet(f'Figure Data/timeseries_{new_mask_filename}_{wetland_poly}.parquet')
                ...
                if maubant==True:
                    ...
                    if plot_spread is True:
                        for idx, row in df_b.iterrows():
                            plt.scatter([x + timedelta(days=np.random.uniform(-1, 1)) for x in [row['Date']] * len(row['PIXEL_WSE'])], row['PIXEL_WSE'] - bias, s=2, alpha=0.05, color="lightblue")  
                    save_df = pd.DataFrame({'Date': df_b['Date'], 'WSE': df_b['WSE'] - bias, 'ST_DEV': df_b['ST_DEV'], 'PIXEL_WSE': [row['PIXEL_WSE'] - bias for idx, row in df_b.iterrows()]})
                    save_df.to_parquet(f'Figure Data/timeseries_maubant_{new_mask_filename}_{wetland_poly}.parquet')
                else:
                    continue
        ...
        if plot_spread is False:
            ...
            plt.savefig(f'{save_folder}/timeseries_{wetland_poly}_{mask_name}_plot_spread.png', bbox_inches='tight', pad_inches=0.1, dpi=600)
        plt.show()
    return None

def plot_all_time_series_gauge_discharge(gauge_filename, discharge_filename, wetland_polygons, timeseries_filename, threshold_days=1, connecting_threshold=21, plot_spread=False):
    '''
    ...
    with open(f"{save_folder}/all_bm_date_list.pkl", "wb") as f:
    ...
    label_list = []
    for wetland_name, swot_df in swot_time_series.items():
        ...
        if plot_spread is True: 
            for idx, row in swot_df.iterrows():
                ax[0].scatter([x + timedelta(days=np.random.uniform(-1, 1)) for x in [row['Date']] * len(row['PIXEL_WSE_KDE'])], row['PIXEL_WSE_KDE'] - bias, s=0.5, alpha=0.01, color=colours[wetland_name])

        for i in range(len(swot_df['Date']) - 1):
            if abs(swot_df['Date'][i + 1] - swot_df['Date'][i]) <= threshold:
                line = ax[0].errorbar([swot_df['Date'][i], swot_df['Date'][i+1]], [swot_df['WSE_adj'][i], swot_df['WSE_adj'][i+1]], fmt='-o', markersize=2, capsize=0, alpha=0.5, markerfacecolor='white', markeredgecolor=colours[wetland_name])
                ax[0].fill_between([swot_df['Date'][i], swot_df['Date'][i+1]], [swot_df['WSE_adj'][i]-swot_df['ST_DEV_KDE'][i], swot_df['WSE_adj'][i+1] - swot_df['ST_DEV_KDE'][i+1]], [swot_df['WSE_adj'][i] + swot_df['ST_DEV_KDE'][i], swot_df['WSE_adj'][i+1] + swot_df['ST_DEV_KDE'][i+1]], alpha=0.2, color=colours[wetland_name])
            else:
                line = ax[0].errorbar(swot_df['Date'][i], swot_df['WSE_adj'][i], fmt='-o', markersize=2, capsize=0, alpha=0.5, markerfacecolor='white', markeredgecolor=colours[wetland_name])
            if wetland_name not in label_list:
                line.set_label(wetland_name)
                label_list.append(wetland_name)

        save_df = pd.DataFrame({"Date": swot_df['Date'], "Water level": swot_df['WSE_adj'], "Standard deviation": swot_df['ST_DEV_KDE'], "Pizel water level": swot_df['PIXEL_WSE_KDE']-bias})
        save_df.to_parquet(f"{save_folder}/all_bm_{wetland_name}_timeseries.parquet")
                ...
                if len(filtered_df) >= 5:
                    for size in [100]:                        
                        ...
                        if not np.isnan(weighted_avg).all():
                            ...
                            anomaly_distributions = pd.DataFrame({"Pixel WSE anomaly": [pixel_height_anomaly], 'Gridded WSE anomaly': [gridded_height_anomaly[~np.isnan(gridded_height_anomaly)]]})
                            anomaly_distributions.to_json(f"{save_folder}/{wetland_poly}_{str(date_obj)[0:10]}.json", orient="records", indent=2)
                            ...
                            for variable, label, cmap in zip([outlier_removed, gridded_height_anomaly], ['Water surface elevation (m)', 'Water surface elevation anomaly (m)'], ['Blues', blue_white_green_cmap]):
                                ...
                                with open(f"{save_folder}/{wetland_poly}_{label}_{str(date_obj)[0:10]}.pkl", "wb") as f:
                                    pickle.dump((lon_x_edges, lat_y_edges, np.flipud(variable)), f)
...
def plot_grid_size_stdev(wetland_polygons, filename_string, poor_coverage_wetland_dictionary):
    ...
    wetland_df = []
    mean_std = pd.DataFrame({'Grid Size': [50, 100, 150, 200, 250, 300, 350, 400, 450, 500]})
    for wetland_poly in wetland_polygons:
    ...
    for i, (wetland_name, df) in enumerate(zip(wetland_polygons, wetland_df)):
        ...
        for idx, size in enumerate([50, 100, 150, 200, 250, 300, 350, 400, 450, 500]):
            ...
            standard_dev_list.append(data.iloc[idx].tolist())
            plt.scatter([size]*len(data.iloc[idx]), data.iloc[idx], s=3, alpha=0.05, color=colours[i])
        plt.plot(grid_list, average_standard_dev, color=colours[i], label=wetland_name, linewidth=2)
        mean_std[f"{wetland_name}: Mean standard deviation"] = average_standard_dev
        mean_std[f"{wetland_name}: Standard deviation list"] = standard_dev_list

    mean_std.to_parquet(f'{save_folder}/grid_size_stdev.parquet')
    plt.legend(loc='best')
...
def kde_histograms(directory, file_list, wetland_polygons, mask_function, bin_number):
    '''
    Calculates and plots gridded water surface elevation and gridded water surface elevation anomalies (relative to the mean gridded water surface elevation) for a given list of files and wetland polygons. 
    Histograms of pixel level and grid level water surface elevation are also plotted. All plots are saved to .png files using the file date, wetland name and plot type. 
    ...
    for filename in file_list:
        ...
        histogram_list, wetland_list = [], []
        fig, ax = plt.subplots(int(len(wetland_polygons)/3), 3, figsize=(10,6), constrained_layout=True)
        ax = ax.flatten()
        for i, (wetland_poly, bounds) in enumerate(zip(wetland_polygons, wetland_bounds)):
            if not swot_polygon.contains(wetland_polygons[wetland_poly]):
                # print(f'Insufficient wetland polygon coverage: skipping {wetland_poly}')
            ...
            if len(latitude) > 0:
                ...
                if len(filtered_df) > 5:
                    print(len(polygon_df['height']), len(filtered_df['height']), np.round((len(filtered_df)/len(polygon_df)*100)), 0)
                    ax[i].hist(polygon_df['height'], bins=bin_number)
                    ax[i].axvline(x=left_tolerance1, color='red', linestyle='--', linewidth=1, label='Lower KDE threshold')
                    ax[i].axvline(x=right_tolerance1, color='red', linestyle='--', linewidth=1, label='Upper KDE threshold')
                    histogram_list.append(polygon_df['height'].tolist())
                    wetland_list.append(wetland_poly)
        fig.supxlabel("Water surface elevation (m)", fontsize=12)
        fig.supylabel("Pixel frequency", fontsize=12)
        plt.show()

        df = pd.DataFrame({"Wetland": wetland_list, "WSE": histogram_list})
        df.to_json(f"{save_folder}/{str(date_obj)[0:10]}.json", orient="records", indent=2)
    return None

##########################################
## Spatial site-level variable plotting ##
##########################################

def mask_flag(flag_values, flag_to_mask):
    '''Makes a boolean mask from an array of quality flag values that contain any of the bits within a specific bit.
    
    Inputs:
        flag_values: array of pixel quality bit flag values
        flag_to_mask: single integer or bitwise operation of multiple bits e.g. 4 | 16

    Returns: array of True/False, where True represents elements containing the specific bit/bits
    '''
    return (flag_values & flag_to_mask) != 0

def mask_with_only_water(classification):
    '''
    Masks out non-water areas from the arrays and retains only water near land and open water.

    Inputs: classification array
    Returns: array of 1 and NaN, where NaN values represent pixels masked out
    '''
    mask = np.zeros(len(classification))
    for i in range(len(classification)):
        if (
            classification[i] in (3, 4)
        ):
            mask[i] = 1
        else:
            mask[i] = np.nan
...
def plot_continuous_variable_basemap(date, lat, lon, variable_dict, wetland_bounds, logger_point):
    '''Plots pixel backscatter or WSE spatially on a basemap, with the location of a water level logger.
    
    Inputs:
        date: date string for plot title
        lat: np array of latitude values
        lon: np array of longitude values
        variable_dict: dict containing one variable ({'variable name': np array of variable}). Variable name must be 'sigma0' or 'height'
        wetland_bounds: list of [xmin, xmax, ymin, ymax] corresponding to the bounds of the wetland polygon
        logger_point: logger location (longitude, latitude)
    '''
    (var_name,) = variable_dict

    fig, ax = plt.subplots(figsize=(18, 10))
    # Plot basemap image
    ax.imshow(img, extent=img_extent)
    # Plot sigma0 or classification scatters
    if var_name == 'sigma0':
        sc = ax.scatter(lon, lat, c=variable_dict['sigma0'], cmap='coolwarm', s=3, vmin=-15, vmax=35)
        cbar = fig.colorbar(sc, ax=ax)
        cbar.set_label(label='Backscatter (dB)', fontsize=12)
        cbar.ax.tick_params(labelsize=12)
    elif var_name == 'height':
        sc = ax.scatter(lon, lat, c=variable_dict['height'], cmap='viridis', s=3, vmin=94, vmax=96)
        cbar = fig.colorbar(sc, ax=ax)
        cbar.set_label(label='Water Surface Elevation (m)', fontsize=12)
        cbar.ax.tick_params(labelsize=12)
    else:
        print('Make sure variable dict contains "sigma0" or "height" keys.')
    
    # Plot water level logger point
    logger_point.plot(ax=ax, color='magenta', edgecolor='white', linewidth=2.5, markersize=100)

    ax.set_xlabel('Longitude', fontsize=12)
    ax.set_ylabel('Latitude', fontsize=12)
    ax.ticklabel_format(style='plain', useOffset=False)
    ax.set_xlim(wetland_bounds[0], wetland_bounds[1])
    ax.set_ylim(wetland_bounds[2], wetland_bounds[3])
    ax.set_title(f'{date}')
    plt.savefig(f'{save_folder}/spatial_{var_name}_{date}.png', dpi=400)

def plot_discrete_scatter_basemap(date, lat, lon, variable_dict, classification_qual_binary_labels, wetland_bounds, logger_point):
    '''Plots pixel classification or classification qual spatially on a basemap, with the location of a water level logger.
    
    Inputs:
        date: date string for plot title
        lat: np array of latitude values
        lon: np array of longitude values
        variable_dict: dict containing one variable ({'variable name': np array of variable}). Variable name must be 'classification' or 'classification_qual'
        classification_qual_binary_labels: labels for binary categories of pixels with and without selected bit flags
        wetland_bounds: list of [xmin, xmax, ymin, ymax] corresponding to the bounds of the wetland polygon
        logger_point: logger location (longitude, latitude)
    '''
    (var_name,) = variable_dict
    ...
    # Plot classification and binary quality bit flag scatters
    if var_name == 'classification':
        # Create binary colourmap
        classification_cmap = ListedColormap(['limegreen', 'gold', 'turquoise', 'dodgerblue', 'darkblue', 'purple', 'grey'])
        bounds = [1, 2, 3, 4, 5, 6, 7, 8]
        norm = BoundaryNorm(bounds, classification_cmap.N)

        # Define classification labels for all possible classifications
        classification_labels = ['Land', 'Land near water', 'Water near land', 'Open water', 'Dark water', 'Low-coherence water near land', 'Open low-coherence water']

        # Plot classification pixels with labels
        sc = ax.scatter(lon, lat, c=variable_dict['classification'], s=3, cmap=classification_cmap, norm=norm)
        cbar = fig.colorbar(sc, ax=ax)
        cbar.set_ticks([1.5, 2.5, 3.5, 4.5, 5.5, 6.5, 7.5])
        cbar.set_ticklabels(classification_labels, fontsize=12)
        
    elif var_name == 'classification_qual':
        # Create binary colourmap
        binary_cmap = ListedColormap(['orange', 'red'])
        bounds = [0, 1, 2]
        norm = BoundaryNorm(bounds, binary_cmap.N)

        # Plot binary classification qual pixels with predefined labels
        sc = ax.scatter(lon, lat, c=variable_dict['classification_qual'], s=3, cmap=binary_cmap, norm=norm)
        cbar = fig.colorbar(sc, ax=ax)
        cbar.set_ticks([0.5, 1.5])
        cbar.set_ticklabels(classification_qual_binary_labels, fontsize=12)
    else:
        print('Make sure variable dict contains "classification" or "classification_qual" keys.')
        return
...
def compute_s2_ndwi(band_3, band_8, ndwi_tif_path):
    '''Computes and saves an NDWI tiff from tiffs of Sentinel band 3 and band 8.
   
    Inputs:
        band_3: Sentinel 2 band 3 tiff
        band_8: Sentinel 2 band 8 tiff
        ndwi_tif_path: path to the computed NDWI tiff
    '''
    with rasterio.open(band_8) as band_8:
        band8 = band_8.read(1).astype('float32')
        profile = band_8.profile
   
    with rasterio.open(band_3) as band_3:
        band3 = band_3.read(1).astype('float32')
    ...
    with rasterio.open(ndwi_tif_path, 'w', **profile) as dst:
        dst.write(ndwi.astype(rasterio.float32), 1)

def plot_tiff(band_3, band_8, date_str, wetland_bounds, ndwi_tif_path, logger_point):
    '''Plots Sentinel 2 NDWI from band 3 and band 8, with the location of a water level logger.
 
    Inputs:
        band_3: Sentinel 2 band 3 tiff
        band_8: Sentinel 2 band 8 tiff
        date_str: string of date of Sentinel 2 file
        wetland_bounds: list of [xmin, xmax, ymin, ymax] corresponding to the bounds of the wetland polygon
        ndwi_tif_path: path to the computed NDWI tiff
        logger_point: logger location (longitude, latitude)
    '''
    # Call function to compute and save the NDWI tiff
    compute_s2_ndwi(band_3, band_8, ndwi_tif_path)
    ...
    with rasterio.open(ndwi_tif_path) as src:
        ...
        fig, ax = plt.subplots(figsize=(18, 10))
        im = ax.imshow(ndwi, cmap='coolwarm_r', extent=extent, vmin=-1, vmax=1)
        cbar = fig.colorbar(im, ax=ax)
        cbar.set_label(label='NDWI', fontsize=12)
        ...
        plt.savefig(f'{save_folder}/spatial_ndwi_{date_str}.png', dpi=400)

def load_and_clip_dea_waterbodies(dea_waterbodies_path):
    '''Loads DEA Waterbodies shapefile, transforms to WGS84 and clips to study site using img_extent constant.
    
    Inputs: 
        dea_waterbodies_path: path to the full DEA Waterbodies shapefile
    '''
    waterbodies_shp = gpd.read_file(dea_waterbodies_path)
    waterbodies_shp_epsg4326 = waterbodies_shp.to_crs('EPSG:4326')
    waterbodies_study_site = waterbodies_shp_epsg4326.clip(mask=[img_extent[0], img_extent[2], img_extent[1], img_extent[3]])
    return waterbodies_study_site

def plot_dea_waterbodies(dea_waterbodies_path, wetland_bounds, logger_point):
    '''Plots DEA Waterbodies shapefile, with the location of a water level logger.
    ...
    Inputs:
        ...
        logger_point: logger location (longitude, latitude)
    '''
    waterbodies_study_site = load_and_clip_dea_waterbodies(dea_waterbodies_path)

    fig, ax = plt.subplots(figsize=(18, 10))
    ax.imshow(img, extent=img_extent)

    # Plot waterbodies shapefile
    waterbodies_study_site.plot(ax=ax, color='skyblue', edgecolor='none', linewidth=1)
    ...
    ax.set_ylim(wetland_bounds[2], wetland_bounds[3])
    ax.ticklabel_format(style='plain', useOffset=False)
    ax.set_title('DEA Waterbodies')
    ax.set_xlabel('Longitude')
    ax.set_ylabel('Latitude')
    plt.savefig(f'{save_folder}/spatial_dea_waterbodies.png', dpi=400)

def plot_pixel_variables_spatially(directory, file_list, wetland_polygons, ndwi_tif_path, ndwi_date, dea_waterbodies_shp, band_3, band_8, logger_point):
    '''Loads and masks pixel cloud data to plot backscatter, binary prior water classification quality flags and water surface elevation spatially, over water classified areas in a wetland polygon. Also plots unmasked classification of all pixels in the wetland polygon.

    Inputs:
        directory: Directory containing SWOT file to plot
        file_list: list of file names to plot
        wetland_polygons: dict of wetland name and Polygon
        ndwi_tif_path: path to computed NDWI tif
        ndwi_date: date of NDWI data
        dea_waterbodies_shp: shapefile of DEA waterbodies
        band_3: Sentinel 2 band 3 tiff
        band_8: Sentinel 2 band 8 tiff
        logger_point: logger location (longitude, latitude)
        '''
    ...
    for filename in file_list:
        ...
        short_date = str(date_obj)[0:19]
        ds1 = nc.Dataset(f)
        swot_polygon = Polygon([(getattr(ds1, 'inner_first_longitude'), getattr(ds1, 'inner_first_latitude')), (getattr(ds1, 'inner_last_longitude'), getattr(ds1, 'inner_last_latitude')), (getattr(ds1, 'outer_last_longitude'), getattr(ds1, 'outer_last_latitude')), (getattr(ds1, 'outer_first_longitude'), getattr(ds1, 'outer_first_latitude'))])

        # Read in data from SWOT PIXC file
        data = read_pixc_values_tide(ds1)

        # Iterate through the wetlands to process and plot PIXC variables
        for i, (wetland_poly, bounds) in enumerate(zip(wetland_polygons, wetland_bounds)):
            if not swot_polygon.contains(wetland_polygons[wetland_poly]):
                print(f'No pixels in the wetland polygon: skipping {wetland_poly}')
                continue
            # Mask to a box around the wetland polygon
            study_site_mask = (data[1] > bounds[2]) & (data[1] < bounds[3]) & (data[2] > bounds[0]) & (data[2] < bounds[1])
            filtered_arrays = [array[study_site_mask] for array in data]
            [height, latitude, longitude, classification, sigma0, geoid, intf_qual, geolocation_qual, pixel_area, classification_qual, sig0_qual, solid_earth_tide, load_tide_fes, load_tide_got, pole_tide] = filtered_arrays

            # Create mask of water near land and open water
            water_mask = mask_with_only_water(classification)

            # Apply mask and tidal models to obtain WSE relative to the EGM2008 geoid for pixels with good quality data
            Masked_Pixels = (height - geoid - solid_earth_tide - load_tide_fes - pole_tide) * water_mask

            Masked_Height = Masked_Pixels[~np.isnan(Masked_Pixels)]
            Masked_latitude = latitude[~np.isnan(Masked_Pixels)]
            Masked_longitude = longitude[~np.isnan(Masked_Pixels)]
            Masked_classification_qual = classification_qual[~np.isnan(Masked_Pixels)]
            Masked_sigma0 = sigma0[~np.isnan(Masked_Pixels)]

            # Filter masked variables to the wetland polygon
            points = shapely.points(Masked_longitude, Masked_latitude)
            tree = STRtree(points)
            indices = tree.query(wetland_polygons[wetland_poly], predicate='contains')
           
            if not len(indices) >= 5:
                print('Less than 5 valid pixels in wetland polygon')
                continue
 
            Good_Pixel_Heights = Masked_Height[indices].compressed()
            Good_lat = Masked_latitude[indices].compressed()
            Good_lon = Masked_longitude[indices].compressed()
            Good_classification_qual = Masked_classification_qual[indices].compressed()
            Good_sigma0 = Masked_sigma0[indices].compressed()

            # Filter unmasked classification to the wetland polygon
            points = shapely.points(longitude, latitude)
            tree = STRtree(points)
            indices = tree.query(wetland_polygons[wetland_poly], predicate='contains')
           
            if not len(indices) >= 5:
                print('Less than 5 unmasked pixels in wetland polygon')
                continue
            Unmasked_lat = latitude[indices].compressed()
            Unmasked_lon = longitude[indices].compressed()
            Unmasked_classification = classification[indices].compressed()

            # Plot the NDWI tif
            plot_tiff(band_3, band_8, ndwi_date, bounds, ndwi_tif_path, logger_point)

            # Clip and plot the DEA Waterbodies shapefile
            plot_dea_waterbodies(dea_waterbodies_shp, bounds, logger_point)

            # Plot a boolean mask of pixels with classification_qual 4 or 16
            Good_classification_qual_contains_4 = mask_flag(Good_classification_qual.astype(int), 4 | 16)
            classification_qual_binary_labels = ['Flag includes "detected water \nbut no prior water" or "water \nfalse detection rate suspect"', 'Flagged as other']

            # Plot unmasked classification (all pixels) on a basemap
            plot_discrete_scatter_basemap(short_date, Unmasked_lat, Unmasked_lon, {'classification': Unmasked_classification}, classification_qual_binary_labels, bounds, logger_point)

            # Plot backscatter and WSE variables on a basemap
            plot_continuous_variable_basemap(short_date, Good_lat, Good_lon, {'sigma0': 10 * np.log10(Good_sigma0 ** 2)}, bounds, logger_point)
            
            plot_discrete_scatter_basemap(short_date, Good_lat, Good_lon, {'classification_qual': Good_classification_qual_contains_4}, classification_qual_binary_labels, bounds, logger_point)

            plot_continuous_variable_basemap(short_date, Good_lat, Good_lon,{'height': Good_Pixel_Heights}, bounds, logger_point)
            
            df_to_save = pd.DataFrame({
                 "Latitude of water pixels": Good_lat,
                 "Longitude of water pixels": Good_lon,
                 "Backscatter (dB) of water pixels": 10 * np.log10(Good_sigma0 ** 2),
                 "Prior water flagged of water pixels": Good_classification_qual_contains_4,
                 "WSE (m) of water pixels": Good_Pixel_Heights
            })
            df_to_save.to_parquet(f"{save_folder}/backscatter_binaryclassqual_wse.parquet")

            df_to_save = pd.DataFrame({
                 "Latitude of all pixels": Unmasked_lat,
                 "Longitude of all pixels": Unmasked_lon,
                 "Classification of all pixels": Unmasked_classification,
            })
            df_to_save.to_parquet(f"{save_folder}/classification.parquet")